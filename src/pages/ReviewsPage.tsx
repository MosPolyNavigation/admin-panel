...
const STATUS_MAP: Record<number, string> = {
  1: "бэклог", 2: "на рассмотрении", 3: "принят в работу",
  4: "ждет проверки", 5: "исправлен", 6: "не требует исправления",
  7: "исправление отложено",
};

const STATUS_COLORS: Record<number, string> = {
  1: "neutral", 2: "primary", 3: "warning",
  4: "info", 5: "success", 6: "success", 7: "danger",
};
...
function ReviewsPage() {
  ...
  const renderPageNumbers = (total: number, current: number, onChange: (page: number) => void) => {
    ...
    const pages = [
      <Button key={1} size="sm" variant={current === 1 ? "solid" : "outlined"} onClick={() => onChange(1)}>1</Button>
    ];
    
    if (current > 3) pages.push(<Typography key="e1" sx={{ alignSelf: 'center' }}>...</Typography>);
    ...
    if (current < total - 2) pages.push(<Typography key="e2" sx={{ alignSelf: 'center' }}>...</Typography>);
    
    pages.push(
      <Button key={total} size="sm" variant={current === total ? "solid" : "outlined"} onClick={() => onChange(total)}>
        {total}
      </Button>
    );
              ...
              <Accordion expanded={expanded[statusId]} onChange={() => toggleExpand(statusId)}>
                ...
                >
                  <Stack direction="row" spacing={2} alignItems="center" sx={{ flex: 1 }}>
                    <Badge badgeContent={statusReviews.length} color={STATUS_COLORS[statusId] as any} size="sm">
                      <Typography level="title-md">Статус: {statusName}</Typography>
                    </Badge>
                    <Chip size="sm" variant="soft">ID: {statusId}</Chip>
                  </Stack>
                </AccordionSummary>
                ...
                <AccordionDetails>
                  {!statusReviews.length ? (
                    <Box sx={{ p: 3, textAlign: 'center' }}>
                      <Typography level="body-md" color="neutral">Нет отзывов в этом статусе</Typography>
                    </Box>
                  ) : (
                    <>
                        ...
                        <Table stickyHeader size="sm">
                          ...
                          <tbody>
                            {currentReviews.map(review => (
                              <tr key={review.id}>
                                <td style={{ padding: '8px' }}><Typography level="body-sm" fontWeight="md">{review.id}</Typography></td>
                                <td style={{ padding: '8px' }}>
                                  <Chip size="sm" variant="soft">{translateProblemId(review.problemId)}</Chip>
                                </td>
                                ...
                                <td style={{ padding: '8px' }}>
                                  {review.imageName ? (
                                    ...
                                    <Chip size="sm" variant="soft" color="neutral">Нет</Chip>
                                  )}
                                </td>
                      ...
                      <Typography level="body-xs" sx={{ mt: 1, textAlign: 'right', color: 'neutral.500' }}>
                        Показано {currentReviews.length} из {statusReviews.length} отзывов • Страница {currentPage} из {totalPages}
                      </Typography>
                    </>
                  )}
                </AccordionDetails>
